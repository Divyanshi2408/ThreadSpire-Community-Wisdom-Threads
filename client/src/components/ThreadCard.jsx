...
//     try {
//       const typeToSend = userReaction === reactionType ? null : reactionType;

//       const res = await reactToThread(thread._id, typeToSend);
...
const ThreadCard = ({ thread }) => {
  ...
  const handleReaction = async (reactionType) => {
    try {
      const res = await reactToThread(thread._id, {
        type: reactionType,
      });
      setReactions(res.data.reactions);
      setUserReaction(res.data.userReaction);
